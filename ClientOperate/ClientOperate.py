# -*- coding: utf-8 -*-
import socket
import time,os,struct,json,rsa
file_dic=['file_encrypted', 'AES_key_encrypted', 'file_iv_encrypted', 'file_signature_encrypted', 'file_signature_encrypted']
class Client:
    def __init__(self):
    ...
    def login(self,username,password):
        ...
        header_hex = bytes(json.dumps(header).encode('utf-8'))  # 信息头
        fhead = struct.pack('1024s', header_hex)                # 信息头16进制转换
        self.ssock.send(fhead)                                  # 将信息头发送过去
        print('send over...')
    ...
    def audit(self):
        ...
        if buf:  # 如果不加这个if，第一个文件传输完成后会自动走到下一句
            ...
            if stat == 'Success':
                fileSize = header['fileSize']
                filenewname = os.path.join(os.path.dirname(__file__)+'/ClientCache/', 'Serverlog.txt')
